...
def train(
    ...
    **kwargs,
    ...
    """Train a DRL model for stock trading.
    ...
    Args:
        ...
        drl_lib: DRL library to use ('elegantrl', 'rllib', 'stable_baselines3').
        env: Environment class to use.
        model_name: Name of the model ('a2c', 'ppo', 'ddpg', 'td3', 'sac').
        if_vix: Whether to include VIX data.
        **kwargs: Additional arguments for data processing and training.
    ...
    if if_vix:
        ...
        # StockTradingEnv always expects a market-risk array. When VIX is
        # disabled (the IDX default), use FinRL's covariance-based turbulence
        # index instead of leaving the required column undefined.
    ...
    if drl_lib == "elegantrl":
        ...
        trained_model.save(cwd)
        logger.info(f"Trained model is saved in {cwd}")
    else:
