...
class AlpacaPaperTrading:
    def __init__(
        ...
        if drl_lib == "stable_baselines3":
            # All Stable-Baselines3 algorithms supported by the training
            # workflow expose the same ``load`` and ``predict`` interface.
            # The original paper-trading implementation unnecessarily
            # restricted this path to PPO.
            ...
            if agent not in sb3_models:
                raise ValueError(
                    f"Stable-Baselines3 agent '{agent}' is not supported. "
                    f"Choose one of: {', '.join(sb3_models)}."
                )
            try:
                self.model = sb3_models[agent].load(cwd)
                print("Successfully loaded model", cwd)
            except (FileNotFoundError, ValueError, OSError) as error:
                raise ValueError(f"Fail to load agent: {error}") from error
            ...
            raise ValueError(
                "Paper trading currently supports PPO for ElegantRL and RLlib. "
                "For Stable-Baselines3, A2C, DDPG, PPO, SAC, and TD3 are supported."
            )
